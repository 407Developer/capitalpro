...
async function detectSchema() {
    ...
    try {
        ...
        console.warn("Schema detection failed.");
    }
}
...
window.syncWithSupabase = async function (silent = false) {
    ...
    try {
        ...
        if (syncBtn) {
            syncBtn.textContent = '☁️ SYNC WITH CLOUD';
        ...
        if (found) {
            ...
            if (!buttonGroup) {
                ...
                buttonGroup.innerHTML = `
                    <button type="button" class="btn btn-secondary" style="flex:1; font-size: 10px; padding: 8px;" onclick="showAddItemModal('${val}')">+ STOCK</button>
                    <button type="button" id="ext-source-btn" class="btn btn-primary" style="flex:1; font-size: 10px; padding: 8px; background: var(--surface);" onclick="toggleExternalSource(true)">EXTERNAL</button>
                `;
...
window.toggleExternalSource = function(isExt) {
    ...
    if (isExt) {
        ...
        costInput.setAttribute('readonly', true);
        if (extBtn) extBtn.style.background = 'var(--surface)';
    }
};
...
window.showAddItemModal = function (initialName) {
    ...
    modal.style = "position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0, 0, 0, 0.8); display:flex; align-items:center; justify-content:center; z-index:2000; backdrop-filter: blur(8px);";
    modal.innerHTML = `
        <div style="background:var(--bg); padding:32px; border-radius:24px; width:90%; max-width:400px; border: 1px solid var(--border); box-shadow: var(--shadow);">
            <h3 style="margin-top:0; margin-bottom: 24px; font-size: 0.8rem; font-weight: 800; color: var(--primary); letter-spacing:0.1em;">NEW INVENTORY ITEM</h3>
            ...
            <div style="display:flex; gap:12px;">
                <button onclick="saveNewInventoryItem()" class="btn btn-primary" style="flex:1;">SAVE</button>
...
window.saveNewInventoryItem = async function () {
    ...
    const newItem = { item_name: name, cost_price: cost, stock_qty: qty, created_at: new Date().toISOString() };
...
window.addToCart = function () {
    ...
    cart.push({ item_name: name, sale_price: sell, quantity: qty, profit: profit, cost_price: cost, is_external: isExternalSource });
...
function renderCart() {
    ...
    cartList.innerHTML = cart.map((item, index) => `
        <li class="cart-item">
            <div class="cart-item-info">
                ...
                <span class="cart-item-price">@ ₦${item.sale_price.toLocaleString()}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 12px;">
                <strong style="font-weight: 700; color: var(--success);">₦${(item.quantity * item.sale_price).toLocaleString()}</strong>
                <button onclick="removeFromCart(${index})" class="cart-remove-btn">✕</button>
            </div>
        </li>
    `).join('');
    ...
    cartTotal.innerHTML = `
        <div style="display: flex; justify-content: space-between; width: 100%; align-items: center; padding-top: 16px; border-top: 1px solid var(--border); margin-top: 8px;">
            <span style="font-size: 0.65rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase;">TOTAL</span>
            <span style="font-size: 1.25rem; font-weight: 800; color: #fff;">₦${total.toLocaleString()}</span>
        </div>
    `;
}
...
window.checkout = async function () {
    ...
    const newSales = cart.map(item => ({ transaction_id: transactionId, customer_name: customerName || 'Walk-in Customer', item_name: item.item_name, sale_price: item.sale_price, quantity: item.quantity, profit: item.profit, cost_price: item.cost_price, is_external: item.is_external, created_at: timestamp }));
    cart.forEach(cartItem => {
        if (!cartItem.is_external) {
            const invIdx = inventory.findIndex(i => i.item_name.toLowerCase() === cartItem.item_name.toLowerCase());
            if (invIdx !== -1) inventory[invIdx].stock_qty -= cartItem.quantity;
        }
    });
...
function setupFilterUI() {
    ...
    const filterHTML = `
        <div class="filter-menu" style="margin-bottom: 20px; display: flex; gap: 8px; justify-content: center; flex-wrap: wrap;">
            <button onclick="setFilter('day')" class="btn btn-secondary ${currentFilter === 'day' ? 'active' : ''}" style="padding: 6px 12px; font-size: 0.65rem; border-radius: 20px;">TODAY</button>
            <button onclick="setFilter('month')" class="btn btn-secondary ${currentFilter === 'month' ? 'active' : ''}" style="padding: 6px 12px; font-size: 0.65rem; border-radius: 20px;">MONTH</button>
            <button onclick="setFilter('all')" class="btn btn-secondary ${currentFilter === 'all' ? 'active' : ''}" style="padding: 6px 12px; font-size: 0.65rem; border-radius: 20px;">ALL</button>
        </div>
    `;
...
function updateDashboard() {
    ...
    const el = { revenue: document.getElementById('revenue'), profit: document.getElementById('profit'), margin: document.getElementById('margin'), capital: document.getElementById('capital'), cogs: document.getElementById('cogs'), ecogs: document.getElementById('ecogs') };
    if(el.revenue) el.revenue.textContent = `₦${totalRevenue.toLocaleString()}`;
    if(el.profit) el.profit.textContent = `₦${totalProfit.toLocaleString()}`;
    if(el.margin) el.margin.textContent = `${profitMargin.toFixed(1)}%`;
    if(el.capital) el.capital.textContent = `₦${shopCap.toLocaleString()}`;
    if(el.cogs) el.cogs.textContent = `₦${internalCogs.toLocaleString()}`;
    if(el.ecogs) el.ecogs.textContent = `₦${externalCogs.toLocaleString()}`;
}
...
function renderSalesCards() {
    ...
    const grouped = filtered.reduce((acc, sale) => {
        ...
        if (!acc[id]) acc[id] = { customer: sale.customer_name || 'Customer', date: sale.created_at, items: [], total_profit: 0, total_amount: 0 };
    ...
    container.innerHTML = sorted.map(tx => `
        <div class="sale-card">
            <div class="sale-card-header">
                <span style="color: #fff;">👤 ${tx.customer}</span>
            ...
            <div style="margin: 8px 0; border-left: 2px solid var(--primary); padding-left: 12px; display: flex; flex-direction: column; gap: 4px;">
                ${tx.items.map(item => `
                    <div style="font-size: 0.8125rem; display: flex; justify-content: space-between;">
                        <span>${item.quantity}x ${item.item_name} <small style="color: var(--text-muted);">@ ₦${item.sale_price.toLocaleString()}</small></span>
                        <span style="color: var(--text-muted);">₦${(item.quantity * item.sale_price).toLocaleString()}</span>
                    </div>
                `).join('')}
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 8px; padding-top: 8px; border-top: 1px solid var(--border);">
                <span class="sale-card-profit" style="font-size: 0.75rem; font-weight:700;">+₦${tx.total_profit.toLocaleString()}</span>
                <span style="font-weight: 800; font-size: 0.9375rem; color: #fff;">₦${tx.total_amount.toLocaleString()}</span>
            </div>
        </div>
...
function updateChart() {
    ...
    const dailyProfit = last7Days.map(date => {
        return sales.filter(s => new Date(s.created_at).toDateString() === date).reduce((sum, s) => sum + (s.profit || 0), 0);
    });
...
window.exportCSV = function () {
    ...
    const rows = sales.map(s => [new Date(s.created_at).toISOString(), s.customer_name || 'Walk-in Customer', s.item_name, s.quantity, s.sale_price, s.cost_price, s.profit, s.is_external ? 'External' : 'Stock']);
    const csvContent = [headers, ...rows].map(e => e.join(",")).join("\n");
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.setAttribute("download", `capital_pro_backup.csv`);
    link.click();
};
...
window.importCSV = function(event) {
    ...
    reader.onload = function(e) {
        ...
        const importedSales = rows.filter(row => row.trim()).map(row => {
            ...
            return { created_at: cols[0], customer_name: cols[1], item_name: cols[2], quantity: parseInt(cols[3]), sale_price: parseFloat(cols[4]), cost_price: parseFloat(cols[5]), profit: parseFloat(cols[6]), is_external: cols[7] === 'External', transaction_id: crypto.randomUUID() };
        });
        if (confirm(`Import ${importedSales.length} sales?`)) {
